...
<details>
  ...
  While not needed for [conformance](/docs/wcag-documents/glossary-of-terms#conformance), if users change their text size, the checkbox size does not change. It might be also hard for users to select or find a small checkbox when the text size is very large. By using relative units instead of pixels, the size of the checkbox will adapt to the text size.
</details>

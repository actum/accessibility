...
If it is not possible to preserve the formatting when the presentation format changes, supplement additional information with text. For instance, you may put a text description saying "All required fields are marked with an asterisk (*)" into the form instructions.
...
The intention here is to re-create a link functionality by using a <code>&lt;span&gt;</code> element instead of <code>&lt;a&gt;</code>. When users click on the text, they get redirected to another page thanks to the <code>&lt;onclick&gt;</code> functionality, however, there are several issues.
...
<details>
	<summary><span>Using heading elements only for visual effects is considered a failure of this success criterion. True or false?</span></summary>
